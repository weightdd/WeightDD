...
/**
 * In many cases like propositional logic or Boolean algebra,
 * the equation (= false <term>) is equivalent to the logical negation (not <term>)
 *
 */
class EvalFalseTransformation(
  ...
  override fun createLatraRewriterBuilder(
    ...
    return object : AbstractLatraRewriterBuilder(match, sparAntlrTreeTriple.sparTree) {
      override fun internalBuild() {
        val term = TransformationUtility.createStringFromNode(match.labelMap["term"].single())
