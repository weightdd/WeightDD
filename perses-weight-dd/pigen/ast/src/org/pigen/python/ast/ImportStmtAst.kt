...
class ImportStmtAst(
  ...
  override fun toSourceCode(stream: PrintStream, indent: Indent, multiLineMode: Boolean) {
    with(stream) {
      print("import ")
      alias.toSourceCode(stream, indent, multiLineMode)
      print("\n")
    }
  }

  companion object {
    fun createWithName(name: Identifier, alias: Identifier? = null): ImportStmtAst {
      return ImportStmtAst(AliasAst(name, alias))
    }
  }
}
