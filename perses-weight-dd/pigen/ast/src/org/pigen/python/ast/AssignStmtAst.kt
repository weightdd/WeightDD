...
class AssignStmtAst(
  val target: AbstractPythonExprAst,
  val value: AbstractPythonExprAst,
  ...
  override fun toSourceCode(stream: PrintStream, indent: Indent, multiLineMode: Boolean) {
    target.toSourceCode(stream, indent, multiLineMode)
    stream.print(" = ")
    value.toSourceCode(stream, indent, multiLineMode)
    stream.print('\n')
  }
}
