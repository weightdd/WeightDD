...
from global_cache_server import GlobalCacheServer, NullServer

__location__ = os.path.realpath(
    os.path.join(os.getcwd(), os.path.dirname(__file__)))

INSTALLS = {
    "perses": os.path.join(__location__, "..", "benchmark", "binaries", "update_perses.sh"),
    "hdd": os.path.join(__location__, "..", "benchmark", "binaries", "update_perses.sh"),
    "creduce": os.path.join(__location__, "..", "benchmark", "binaries", "update_creduce.sh"),
    "chisel": os.path.join(__location__, "..", "benchmark", "binaries", "update_chisel.sh"),
    "perses-token-replacement": os.path.join(__location__, "..", "benchmark", "binaries", "update_atn_perses_main.sh"),
    "perses-token-pattern": os.path.join(__location__, "..", "benchmark", "binaries", "update_atn_perses_main.sh"),
    "atn-fine-tuning": os.path.join(__location__, "..", "benchmark", "binaries", "update_atn_perses_main.sh")
}

REDUCERS = {
    "perses": os.path.join(__location__, "..", "benchmark", "binaries", "run_perses.sh"),
    "hdd": os.path.join(__location__, "..", "benchmark", "binaries", "run_hdd.sh"),
    "perses-fix": os.path.join(__location__, "..", "benchmark", "binaries", "run_perses_fix.sh"),
    "hdd-fix": os.path.join(__location__, "..", "benchmark", "binaries", "run_hdd_fix.sh"),
    "creduce": os.path.join(__location__, "..", "benchmark", "binaries", "run_creduce.sh"),
    "chisel": os.path.join(__location__, "..", "benchmark", "binaries", "run_chisel.sh"),
    "atn-fine-tuning": os.path.join(__location__, "..", "benchmark", "binaries", "run_atn_fine_tuning.sh"),
}
...
def parse_arguments():
    ...
    parser.add_argument("-c", "--enable-cache", action="store_true", default=False,
                        help="When this flag is enabled, the results of property tests will be cached, "
                             "and the property test won't execute if the results have already been cached.")
    parser.add_argument("-cff", "--cache-file-path", default=None, type=str,
                        help="Specify the path of the cache file.")
...
class Parameter:
    ...
    def validate(self):
        ...
        if self.cache_file_path:
            self.cache_file_path= os.path.abspath(self.cache_file_path)
            cache_file_folder = os.path.dirname(self.cache_file_path)
...
def load_token_counter(subprocess_flag: bool):
    ...
    proc = subprocess.run(
        [os.path.join(__location__, "..", "benchmark", "binaries", "update_token_counter.sh")],
        check=False,
        stdout=pipe,
        stderr=pipe,
        )
...
def count_token(source_file_path: str) -> int:
    try:
        token_counter_sh = os.path.join(__location__, "..", "benchmark", "binaries", "run_token_counter.sh")
...
def environment_updater(
        memory_flag: bool,
        filename_log: str,
        wrapped_test_script_path: str,
    ...
    environment['SOURCE_NAME'] = f"{os.path.basename(source_file_path)}"
    if wrapped_test_script_path != None:
        environment['WRAPPED_SCRIPT_PATH'] = f"{os.path.abspath(wrapped_test_script_path)}"
        environment['WRAPPED_SCRIPT_NAME'] = f"{os.path.basename(wrapped_test_script_path)}"
    if port_number != None:
        environment['CACHE_QUERY_SOCKET_PORT'] = f"{port_number}"
    return environment
...
def report_generator(subject, reducer, reducer_flags, token_count, iteration, run_result: list) -> Dict:
...
def main(parameter: Parameter):
    ...
    for subject_name in parameter.benchmark_target:
        ...
        default_cache_file_path = os.path.join(__location__, subject_name, "lookup_table.json")

        cache_file_path = default_cache_file_path if parameter.cache_file_path == None\
            else parameter.cache_file_path

        # count subject tokens
        token_count = count_token(source_file_path)
        print(f"Subject {subject_name} has {token_count} original tokens")

        wrapped_test_script_path = None
        cache_server = NullServer()
        if parameter.enable_cache:
            if cache_file_path == None:
                raise Exception("Error, cache_file is not designated in info.properties")
            try:
                with open (cache_file_path, 'r') as f:
                    database = json.load(f)
            except Exception:
                database = dict()
            wrapped_test_script_path = script_file_path
            script_file_path = os.path.join(__location__, "property_test_script_wrapper.sh")
            cache_server = GlobalCacheServer(database)
            cache_server.start()
        ...
        for reducer in parameter.reducers:
            ...
            for iteration in range(parameter.iterations):
                ...
                environment = environment_updater(
                    parameter.memory_profiler,
                    filename_log,
                    wrapped_test_script_path,
                    source_file_path,
                    cache_server.get_socket_port_number()
                )
                ...
                # add the time saved by the cache server
                # FIXME: record more precise time
                run_result[4] = str(math.ceil(int(run_result[4]) + cache_server.get_saved_time()))
        ...
        cache_server.save_database(cache_file_path)
    return report
